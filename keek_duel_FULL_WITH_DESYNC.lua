-- FULL MERGED FILE WITH DESYNC (E KEY)
-- (Your original script + desync system added)
...
local desyncAnimConnection = nil
...
local function enableDesync()
    local hum = getHumanoidDesync()

    stopDesyncAnimations(hum)

    if desyncAnimConnection then
        desyncAnimConnection:Disconnect()
    end

    desyncAnimConnection = hum.AnimationPlayed:Connect(function(track)
        track:Stop()
    end)
