...
local configFile = "KeekDuel_config.json"  -- Changed to .json for better compatibility
...
pGradient.Color = ColorSequence.new(Color3.fromRGB(255, 0, 0), Color3.fromRGB(200, 0, 0))
...
fillGradient.Color = ColorSequence.new(Color3.fromRGB(255, 0, 0), Color3.fromRGB(200, 0, 0))
...
ProgressPercentLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
...
rightColumn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
...
rightScroll.BackgroundTransparency = 0.3
...
leftColumn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
...
leftScroll.BackgroundTransparency = 0.3
...
local function makeToggleHeader(parent, labelText)
    ...
    fStroke.Color = Color3.fromRGB(255, 0, 0)

    table.insert(blueBoxes, fStroke)
    ...
    local function updateVisual()
        if isOn then
            TweenService:Create(bg, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0,255,0)}):Play()
...
local function createSlider(parent, labelText, minVal, maxVal, valueKey, onChange)
    ...
    table.insert(blueBoxes, cStroke)
    ...
    table.insert(blueSliders, {sliderFill = sliderFill, thumb = thumb, valueLabel = valueLabel})
...
do
    ...
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "Speed Boost  [" .. speedKeybind.Name .. "]")
    ...
    table.insert(blueBoxes, kbStroke)
...
createSlider(rightScroll, "Speed Value", 1, 70, "BoostSpeed")

do
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "Anti Ragdoll")
...
local function startMeleeAimbot()
    ...
    Cebo.Circle.Color = Color3.new(0, 1, 0)
...
do
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "Hit Circle")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
    ...
    ToggleControls.MeleeAimbot = {setVisual = setVisual, start = startMeleeAimbot, stop = stopMeleeAimbot}
end
...
do
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "SpinBot")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
        Features.SpinBot = on
...
createSlider(rightScroll, "SpinBot Speed", 5, 50, "SpinSpeed", function(v)
    Values.SpinSpeed = v
    applySpinBotSpeed()
end)
...
do
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "Inf Jump")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
        Features.InfJump = on
...
do
    local btn, setVisual, getState = makeToggleHeader(rightScroll, "Auto Steal")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
...
createSlider(rightScroll, "Steal Radius", 1, 30, "STEAL_RADIUS", function(v)
...
do
    ...
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Bat Aimbot  [" .. batKeybind.Name .. "]")

    local frame = btn.Parent
    local lbl = frame:FindFirstChildOfClass("TextLabel")

    local function updateBatLabel()
        if lbl then
            lbl.Text = "Bat Aimbot  [" .. batKeybind.Name .. "]"
        end
    end
    updateBatLabel()
    ...
    local function toggleBatAimbot()
        ...
        local on = not getState()
        setVisual(on)
        Features.BatAimbot = on
        if on then startBatAimbot() else stopBatAimbot() end
    end

    btn.MouseButton1Click:Connect(toggleBatAimbot)
    ...
    ToggleControls.BatAimbot = {setVisual = setVisual, start = startBatAimbot, stop = stopBatAimbot}
end
...
do
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Thief Speed")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
...
createSlider(leftScroll, "Steal Speed", 10, 50, "StealingSpeedValue", function(v)
...
do
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Unwalk")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
        Features.Unwalk = on
...
do
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Optimizer")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
...
do
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "XRay")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
        Features.XRay = on
...
do
    ...
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Float  [" .. floatKeybind.Name .. "]")
    ...
    kbBtnF.TextColor3 = Color3.fromRGB(255, 0, 0)
    ...
    kbStrokeF.Color = Color3.fromRGB(255, 0, 0)
    table.insert(blueBoxes, kbStrokeF)
...
do
    ...
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Right Steal  [" .. rightKeybind.Name .. "]")
    local frame = btn.Parent
    local lbl = frame:FindFirstChildOfClass("TextLabel")
    ...
    kbBtn1.TextColor3 = Color3.fromRGB(255, 0, 0)
    ...
    kbStroke1.Color = Color3.fromRGB(255, 0, 0)
    table.insert(blueBoxes, kbStroke1)
    ...
    local function toggleRight()
        ...
        local on = not getState()
        setVisual(on)
        Features.RightSteal = on
        if Features.LeftSteal then
            Features.LeftSteal = false
            if ToggleControls.LeftSteal then ToggleControls.LeftSteal.setVisual(false) end
        end
        if on then startStealPath(stealPath1) else stopStealPath() end
    end
    ...
    ToggleControls.RightSteal = {setVisual = setVisual, start = function() startStealPath(stealPath1) end, stop = stopStealPath}
end

do
    local leftListening = false

    local btn2, setVisual2, getState2 = makeToggleHeader(leftScroll, "Left Steal  [" .. leftKeybind.Name .. "]")
    ...
    kbBtn2.TextColor3 = Color3.fromRGB(255, 0, 0)
    ...
    kbStroke2.Color = Color3.fromRGB(255, 0, 0)
    table.insert(blueBoxes, kbStroke2)
    ...
    local function toggleLeft()
        ...
        setVisual2(on)
        Features.LeftSteal = on
        if Features.RightSteal then
            Features.RightSteal = false
            if ToggleControls.RightSteal then ToggleControls.RightSteal.setVisual(false) end
        end
        if on then startStealPath(stealPath2) else stopStealPath() end
    end
    ...
    ToggleControls.LeftSteal = {setVisual = setVisual2, start = function() startStealPath(stealPath2) end, stop = stopStealPath}
end
...
do
    local btn, setVisual, getState = makeToggleHeader(leftScroll, "Walk Fling")
    btn.MouseButton1Click:Connect(function()
        local on = not getState()
        setVisual(on)
...
do
    ...
    sStroke.Color = Color3.fromRGB(255, 0, 0)
    table.insert(blueBoxes, sStroke)
...
--// TAUNT FEATURE (Added)
do
    local tauntBtn = Instance.new("TextButton", leftScroll)
    tauntBtn.Size = UDim2.new(1, -10 * guiScale, 0, 40 * guiScale)
    tauntBtn.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    tauntBtn.BackgroundTransparency = 0.3
    tauntBtn.Text = "Taunt"
    tauntBtn.TextColor3 = Color3.fromRGB(255, 0, 0)
    tauntBtn.Font = Enum.Font.GothamBlack
    tauntBtn.TextSize = 12 * guiScale
    tauntBtn.BorderSizePixel = 0
    tauntBtn.LayoutOrder = 999

    Instance.new("UICorner", tauntBtn).CornerRadius = UDim.new(0, 8 * guiScale)

    local stroke = Instance.new("UIStroke", tauntBtn)
