...
local function saveConfig()
	...
	if writefile then pcall(function() writefile("KeekHubConfig.json",HttpService:JSONEncode(cfg)) end) end
end
...
local function setupChar(char)
	...
	if head then
		local old=head:FindFirstChild("KeekSpeedBB");if old then old:Destroy() end
		local bb=Instance.new("BillboardGui",head)
		bb.Name="KeekSpeedBB";bb.Size=UDim2.new(0,140,0,25)
...
-- ===================== GUI CLEANUP =====================
for _,n in pairs({"KeekHubGUI"}) do
...
gui.Name="KeekHubGUI";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
...
titleLbl.BackgroundTransparency=1;titleLbl.Text="KEEK HUB"
...
-- removed old mini toggle


-- ===== PREMIUM TOGGLE BUTTON =====
local toggleBtn = Instance.new("ImageButton", gui)
toggleBtn.Name = "PremiumToggle"
...
local glow = Instance.new("UIStroke", toggleBtn)
glow.Thickness = 6
...
local function loadConfig()
	if not(isfile and isfile("KeekHubConfig.json")) then return end
	local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("KeekHubConfig.json")) end)
...
buildGUI()
print("[Keek Hub v1] Loaded!")


-- ====================== MOBILE BUTTONS (NEW SYSTEM) ======================
local DARK_ON=Color3.fromRGB(20,20,25)
local BLACK_OFF=Color3.fromRGB(8,8,8)

local panelGui=Instance.new("ScreenGui",player:WaitForChild("PlayerGui"))
panelGui.Name="KeekDuel_MobilePanel"
panelGui.ResetOnSpawn=false
panelGui.IgnoreGuiInset=true
panelGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
panelGui.DisplayOrder=8

local BTN_W, BTN_H = 52, 50
local COL_PAD, ROW_PAD = 12, 10
local PANEL_X_OFF, PANEL_Y_OFF = -128, -130

local function cellPos(col, row)
    local x = PANEL_X_OFF + col * (BTN_W + COL_PAD)
    local y = PANEL_Y_OFF + row * (BTN_H + ROW_PAD)
    return UDim2.new(1, x, 0.5, y)
end

local function makePBtn(l1, l2, initialPos)
    local holder=Instance.new("Frame",panelGui)
    holder.Size=UDim2.new(0,BTN_W,0,BTN_H)
    holder.Position=initialPos
...
local btnAAL = makePBtn("AUTO","L",cellPos(0,0))
local btnAAR = makePBtn("AUTO","R",cellPos(1,0))
local btnPL  = makePBtn("PLAY","L",cellPos(0,1))
local btnPR  = makePBtn("PLAY","R",cellPos(1,1))
local btnDR  = makePBtn("DROP","",cellPos(0,2))
local btnTP  = makePBtn("TP","DOWN",cellPos(1,2))
local btnLK  = makePBtn("LOCK","",cellPos(0,3))
local btnCS  = makePBtn("CARRY","SPD",cellPos(1,3))
