...
local desyncMoveConn = nil

local function startDesyncMovement()
    if desyncMoveConn then return end
    desyncMoveConn = RunService.Heartbeat:Connect(function()
        ...
        local speed = Config.DesyncNormalSpeed
        if isCarryingBrainrot() then
            speed = Config.DesyncCarrySpeed
        end
...
local function stopDesyncMovement()
    if desyncMoveConn then
        desyncMoveConn:Disconnect()
        desyncMoveConn = nil
    end
end
...
local function isCarryingBrainrot()
    ...
    return tool.Name:lower():find("brainrot") ~= nil
end
