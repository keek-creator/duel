...
--// ==================== MELEE AIMBOT (NEW LOCK) ====================
local MELEE_RANGE = 45
local MELEE_ONLY_ENEMIES = true

local meleeEnabled = false
local meleeConnection = nil
local meleeAlignOrientation = nil
local meleeAttachment = nil
...
local function createMeleeAimbot(char)
    ...
    meleeConnection = RunService.RenderStepped:Connect(function()
        if not meleeEnabled or not char.Parent then
...
local function disableMeleeAimbot()
    ...
    if meleeAlignOrientation then
        meleeAlignOrientation:Destroy()
        meleeAlignOrientation = nil
    end

    if meleeAttachment then
        meleeAttachment:Destroy()
        meleeAttachment = nil
    end
...
btnLock.MouseButton1Click:Connect(function() if not checkLock() then return end; SETTINGS.LOCK_ENABLED = not SETTINGS.LOCK_ENABLED; setVisual(btnLock, gradLock, strokeLock, SETTINGS.LOCK_ENABLED, "LOCK: ON", "LOCK: OFF"); 
            end
