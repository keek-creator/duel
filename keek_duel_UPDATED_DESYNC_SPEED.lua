...
--// DESYNC VARIABLES //
local DesyncNormalSpeed = 50 -- default
...
--// ==================== DESYNC CORE ====================
local function getDesyncHumanoid()
	local char = player.Character or player.CharacterAdded:Wait()
	return char:WaitForChild("Humanoid")
end

local function stopDesyncAnimations(hum)
...
local function enableDesync()
	local humanoid = getDesyncHumanoid()
	stopDesyncAnimations(humanoid)

	desyncConnection = humanoid.AnimationPlayed:Connect(function(track)
		track:Stop()
	end)
	...
	RunService.Heartbeat:Connect(function()
		...
		if hrp then
			...
			hrp.AssemblyLinearVelocity = Vector3.new(
				vel.X * (DesyncNormalSpeed / 10),
				vel.Y,
				vel.Z * (DesyncNormalSpeed / 10)
			)
		end
	end)
end
...
local desyncToggle, desyncInd, desyncBase = createToggle("DESYNC")
...
desyncToggle.MouseButton1Click:Connect(function()
	toggleDesync()
	updateVisual(desyncEnabled, desyncInd, desyncBase)
end)
...
-- DESYNC SPEED INPUT
local desyncBox = Instance.new("TextBox")
desyncBox.Size = UDim2.new(0.9, 0, 0, 30)
desyncBox.BackgroundColor3 = Color3.fromRGB(20,20,20)
desyncBox.TextColor3 = Color3.fromRGB(255,255,255)
desyncBox.PlaceholderText = "Desync Speed"
desyncBox.Text = tostring(DesyncNormalSpeed)
desyncBox.Font = Enum.Font.GothamBold
desyncBox.TextSize = 14
desyncBox.Parent = container
Instance.new("UICorner", desyncBox).CornerRadius = UDim.new(0,6)

desyncBox.FocusLost:Connect(function()
	local val = tonumber(desyncBox.Text)
	if val then
		DesyncNormalSpeed = math.clamp(val, 1, 200)
	else
		desyncBox.Text = tostring(DesyncNormalSpeed)
	end
end)
