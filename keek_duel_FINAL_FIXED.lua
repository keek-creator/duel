...
local function startFloat()
    ...
    local hum = getHum()
    if hum then
        if isCarryingAnimal() then
            hum.WalkSpeed = CARRY_SPEED
        else
...
local function startAutoRight()
    ...
    if hum then
        ...
        local speed = isCarryingAnimal() and CARRY_SPEED or NORMAL_SPEED
hum:Move(moveDir * (speed / 16), false)
...
-- SPEED INPUTS
createInput("Walk Speed", NORMAL_SPEED, function(v)
    NORMAL_SPEED = v
end)

createInput("Carry Speed", CARRY_SPEED, function(v)
    CARRY_SPEED = v
end)
