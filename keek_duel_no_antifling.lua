...
local function startAutoRight()
    ...
    autoRightConn = RunService.Heartbeat:Connect(function()
        ...
        local speed = Config.ForwardSpeed

-- if target is one of your special coordinates
if (target - Vector3.new(-474.67, -6.94, 105.48)).Magnitude < 1
or (target - Vector3.new(-474.22, -6.96, 16.18)).Magnitude < 1 then
    speed = 29
end

local speed = (autoRightIndex >= 3) and Config.SlowSpeed or Config.ForwardSpeed
...
local speed = (autoLeftIndex >= 3) and Config.SlowSpeed or Config.ForwardSpeed
...
local function startAutoSwing()
    ...
    task.spawn(function()
        while autoSwingActive and SETTINGS.LOCK_ENABLED do
            equipAnyTool(); silentSwing(); task.wait(math.random(30, 50) / 100)
        end
...
local function createFloatingBar(name, defaultText, startPos)
    ...
    frame.Name = name; frame.Size = UDim2.new(0, 160, 0, 40); frame.Position = startPos; frame.BackgroundColor3 = Color3.fromRGB(255, 255, 255); frame.BorderSizePixel = 0; frame.Active = true; frame.Parent = sg or screenGui
    local uiGradient = Instance.new("UIGradient")
    uiGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(26, 11, 46)), ColorSequenceKeypoint.new(1, Color3.fromRGB(5, 5, 20))}; uiGradient.Rotation = 45; uiGradient.Parent = frame
    local uiCorner = Instance.new("UICorner"); uiCorner.CornerRadius = UDim.new(0, 8); uiCorner.Parent = frame
    local uiStroke = Instance.new("UIStroke"); uiStroke.Color = Color3.fromRGB(138, 43, 226); uiStroke.Thickness = 2; uiStroke.Transparency = 0.2; uiStroke.Parent = frame
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(1, 0, 1, 0); btn.BackgroundTransparency = 1; btn.Text = defaultText; btn.TextColor3 = Color3.fromRGB(200, 180, 255); btn.Font = Enum.Font.GothamBold; btn.TextSize = 13; btn.Parent = frame
    local dragging, dragInput, dragStart, startPosFrame, hasDragged = false
    btn.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = true; hasDragged = false; dragStart = input.Position; startPosFrame = frame.Position; input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false end end) end end)
    btn.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then dragInput = input; if dragging then local delta = (input.Position - dragStart).Magnitude; if delta > 5 then hasDragged = true end end end end)
    UserInputService.InputChanged:Connect(function(input) if input == dragInput and dragging then local delta = input.Position - dragStart; frame.Position = UDim2.new(startPosFrame.X.Scale, startPosFrame.X.Offset + delta.X, startPosFrame.Y.Scale, startPosFrame.Y.Offset + delta.Y) end end)
    local function isRealClick() return not hasDragged end
...
local function setVisual(btn, grad, stroke, state, onTxt, offTxt)
    btn.Text = state and onTxt or offTxt; btn.TextColor3 = state and Color3.new(1,1,1) or Color3.fromRGB(200, 180, 255)
    grad.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, state and Color3.fromRGB(65, 20, 100) or Color3.fromRGB(26, 11, 46)), ColorSequenceKeypoint.new(1, state and Color3.fromRGB(20, 50, 80) or Color3.fromRGB(5, 5, 20))}
    stroke.Color = state and Color3.fromRGB(50, 255, 150) or Color3.fromRGB(138, 43, 226)
end
...
btnLock.MouseButton1Click:Connect(function() if not checkLock() then return end; SETTINGS.LOCK_ENABLED = not SETTINGS.LOCK_ENABLED; setVisual(btnLock, gradLock, strokeLock, SETTINGS.LOCK_ENABLED, "LOCK: ON", "LOCK: OFF"); if SETTINGS.LOCK_ENABLED then equipAnyTool(); startAutoSwing() end end)
