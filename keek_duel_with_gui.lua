...
--// ===== FULL CUSTOM AUTO PLAY SYSTEM (FINAL) ===== //
...
-- GUI BUTTONS FOR SAVING POINTS
task.spawn(function()
    task.wait(2)
    if autoPanel then
        local function makeBtn(name, x, y, isRight)
            local b = Instance.new("TextButton")
            b.Size = UDim2.new(0.22,0,0,18)
        ...
        -- LEFT
        makeBtn("L1",0.02,0,false)
        makeBtn("L2",0.26,0,false)
        makeBtn("L3",0.50,0,false)
        makeBtn("L4",0.74,0,false)

        -- RIGHT
        makeBtn("R1",0.02,0.3,true)
        makeBtn("R2",0.26,0.3,true)
        makeBtn("R3",0.50,0.3,true)
        makeBtn("R4",0.74,0.3,true)
    end
end)

-- OVERRIDE AUTO BUTTON
task.spawn(function()
    task.wait(2)
    if btnAuto then
...
--// ===== ADDED GUI CONTROLS (L1-L4 / R1-R4 + AUTOPLAY) ===== //--
...
-- buttons
local L1 = makeBtn("L1", 10, 60)
local L2 = makeBtn("L2", 10, 100)
local L3 = makeBtn("L3", 10, 140)
local L4 = makeBtn("L4", 10, 180)

local R1 = makeBtn("R1", 130, 60)
local R2 = makeBtn("R2", 130, 100)
local R3 = makeBtn("R3", 130, 140)
local R4 = makeBtn("R4", 130, 180)

-- auto section
local autoLabel = Instance.new("TextLabel")
autoLabel.Text = "Auto Play"
autoLabel.Size = UDim2.new(1,0,0,20)
autoLabel.Position = UDim2.new(0,0,0,220)
...
autoBtn.Position = UDim2.new(0, 20, 0, 250)
autoBtn.Text = "AUTO PLAY: OFF"
autoBtn.Parent = frame

autoBtn.MouseButton1Click:Connect(function()
    autoPlay = not autoPlay
    autoBtn.Text = autoPlay and "AUTO PLAY: ON" or "AUTO PLAY: OFF"
end)

-- TODO: Replace this with your actual function
local function trigger(skill)
    print("Used:", skill)
end

-- button connections
L1.MouseButton1Click:Connect(function() trigger("L1") end)
L2.MouseButton1Click:Connect(function() trigger("L2") end)
L3.MouseButton1Click:Connect(function() trigger("L3") end)
L4.MouseButton1Click:Connect(function() trigger("L4") end)

R1.MouseButton1Click:Connect(function() trigger("R1") end)
R2.MouseButton1Click:Connect(function() trigger("R2") end)
R3.MouseButton1Click:Connect(function() trigger("R3") end)
R4.MouseButton1Click:Connect(function() trigger("R4") end)

-- autoplay loop
task.spawn(function()
    while true do
        if autoPlay then
            trigger("L1") task.wait(0.3)
            trigger("L2") task.wait(0.3)
            trigger("L3") task.wait(0.3)
            trigger("L4") task.wait(0.3)
            trigger("R1") task.wait(0.3)
            trigger("R2") task.wait(0.3)
            trigger("R3") task.wait(0.3)
            trigger("R4") task.wait(0.3)
        end
        task.wait(0.1)
    end
end)
