...
--// ===== CUSTOM L1-L4 / R1-R4 SYSTEM (ADDED) ===== //

local AutoMode = "RIGHT"
local leftPoints = {nil, nil, nil, nil}
local rightPoints = {nil, nil, nil, nil}
local selectedSlot = 1

-- Override AutoPlay to use slots
local autoCustomConn
local function getPoints()
    return (AutoMode == "LEFT") and leftPoints or rightPoints
end

local function startCustomAuto()
    if autoCustomConn then autoCustomConn:Disconnect() end
    local currentIndex = 1

    autoCustomConn = RunService.Heartbeat:Connect(function()
        ...
        if not target then
            currentIndex += 1
            if currentIndex > 4 then currentIndex = 1 end
            return
        end
        ...
        hrp.AssemblyLinearVelocity = Vector3.new(
            moveDir.X * Config.AutoPlaySpeed,
            hrp.AssemblyLinearVelocity.Y,
            moveDir.Z * Config.AutoPlaySpeed
        )
    end)
end

local function stopCustomAuto()
    if autoCustomConn then
        autoCustomConn:Disconnect()
        autoCustomConn = nil
    end
end
