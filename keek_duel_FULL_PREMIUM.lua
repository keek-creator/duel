...
--// CLEAN SPEED PANEL UI (UPGRADED) //
autoPanel.Size = UDim2.new(0, 160, 0, 90)

local function createInputBox(placeholder, defaultText, yPos)
...
local speedBox = createInputBox("Auto Speed", tostring(Config.AutoPlaySpeed), 5)
local carryBox = createInputBox("Carry Speed", tostring(Config.CarrySpeed), 35)

speedBox.FocusLost:Connect(function()
    local val = tonumber(speedBox.Text)
    if val then
        Config.AutoPlaySpeed = val
        speedBox.TextColor3 = Color3.fromRGB(120,255,120)
...
carryBox.FocusLost:Connect(function()
    ...
    if val then
        Config.CarrySpeed = val
        carryBox.TextColor3 = Color3.fromRGB(120,255,120)
        task.delay(0.2, function()
            carryBox.TextColor3 = Color3.fromRGB(220,220,220)
        end)
    end
end)
...
local function setVisual(btn, grad, stroke, state, onTxt, offTxt)
    ...
    TweenService:Create(stroke, TweenInfo.new(0.25), {
        Color = state and Color3.fromRGB(120,120,120) or Color3.fromRGB(60,60,60)
    }):Play()

    grad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, state and Color3.fromRGB(35,35,35) or Color3.fromRGB(20,20,20)),
        ColorSequenceKeypoint.new(1, state and Color3.fromRGB(15,15,15) or Color3.fromRGB(10,10,10))
    }
end
