...
btnLock.MouseButton1Click:Connect(function() if not checkLock() then return end; SETTINGS.LOCK_ENABLED = not SETTINGS.LOCK_ENABLED; setVisual(btnLock, gradLock, strokeLock, SETTINGS.LOCK_ENABLED, "LOCK: ON", "LOCK: OFF"); if SETTINGS.LOCK_ENABLED then lockTarget(); equipAnyTool(); startAutoSwing() else unlockTarget() end end)
