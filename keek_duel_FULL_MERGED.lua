...
local COL_ROW = Color3.fromRGB(25,25,30)
local COL_TOGGLE = Color3.fromRGB(40,40,50)
local COL_OUTLINE = Color3.fromRGB(20,20,25)
...
if CoreGui:FindFirstChild("LaggerUI") then
	CoreGui.LaggerUI:Destroy()
end
...
close.Text = "X"
close.BackgroundColor3 = COL_TOGGLE
...
toggle(35,"Lag",function(v) lagEnabled=v end)
toggle(65,"Speed",function(v) speedEnabled=v end)

-- SLIDERS
local function slider(y,text,setPercent)
	...
	knob.Position = UDim2.new(percent/100,0,0.5,0)
	knob.BackgroundColor3 = Color3.new(1,1,1)
	Instance.new("UICorner", knob)
	...
	local function update(x)
		...
		lbl.Text = text..": "..string.format("%.1f",percent).."%"
		setPercent(percent)
	end
...
slider(100,"Lag",function(p)
	lagAmount = (p/100) * 0.2
end)

slider(140,"Speed",function(p)
	speedAmount = (p/100) * 70
end)
	...
	if speedEnabled then
		...
		if c then
			...
			if hrp and hum and hum.MoveDirection.Magnitude>0 then
				local d=hum.MoveDirection.Unit
				hrp.AssemblyLinearVelocity=Vector3.new(d.X*speedAmount,hrp.AssemblyLinearVelocity.Y,d.Z*speedAmount)
			end
		end
	end
end)
