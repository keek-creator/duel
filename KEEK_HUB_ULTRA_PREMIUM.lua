...
-- ===================== KEEK HUB ULTRA PREMIUM PATCH =====================
-- This patch upgrades visuals (glass, gradients, premium toggle, icons, notifications)
-- It is designed to be appended AFTER the GUI is built (buildGUI()) so it can hook existing objects.
...
task.spawn(function()
    ...
    -- wait for GUI
    local gui = (game:GetService("CoreGui"):FindFirstChild("KEEKHubGUI")
        or game:GetService("CoreGui"):FindFirstChild("EnvyHubGUI")
        or LP:WaitForChild("PlayerGui"):FindFirstChild("KEEKHubGUI")
        or LP:WaitForChild("PlayerGui"):FindFirstChild("EnvyHubGUI"))

    if not gui then
        repeat task.wait() 
            gui = (game:GetService("CoreGui"):FindFirstChild("KEEKHubGUI")
                or game:GetService("CoreGui"):FindFirstChild("EnvyHubGUI")
                or LP:FindFirstChild("PlayerGui") and (LP.PlayerGui:FindFirstChild("KEEKHubGUI") or LP.PlayerGui:FindFirstChild("EnvyHubGUI")))
        until gui
    end
    ...
    -- ===== Rename to KEEK (non-destructive: only visible texts) =====
    for _,d in ipairs(gui:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            d.Text = d.Text:gsub("ENVY","KEEK"):gsub("Envy","Keek")
        end
    end
    gui.Name = "KEEKHubGUI"
    ...
    task.spawn(function()
        while main.Parent do
            ...
            task.wait(1)
            TweenService:Create(stroke, TweenInfo.new(1), {Transparency = 0.85}):Play()
            task.wait(1)
        end
    end)

    -- ===== Blur =====
    local blur = Lighting:FindFirstChild("KEEK_Blur") or Instance.new("BlurEffect")
    blur.Name = "KEEK_Blur"
    blur.Size = 0
    ...
    toggle.Name = "KEEK_Toggle"
    toggle.Size = UDim2.new(0, 50, 0, 50)
    toggle.Position = UDim2.new(0, 20, 0.5, -25)
    toggle.BackgroundColor3 = Color3.fromRGB(15,15,15)
    toggle.Image = "rbxassetid://7733715400"
    ...
    local tgrad = Instance.new("UIGradient", toggle)
    ...
    toggle.MouseEnter:Connect(function()
        TweenService:Create(toggle, TweenInfo.new(0.15), {Size = UDim2.new(0,60,0,60)}):Play()
        TweenService:Create(glow, TweenInfo.new(0.2), {Transparency = 0.6}):Play()
    end)

    toggle.MouseLeave:Connect(function()
        TweenService:Create(toggle, TweenInfo.new(0.15), {Size = UDim2.new(0,50,0,50)}):Play()
        TweenService:Create(glow, TweenInfo.new(0.2), {Transparency = 0.9}):Play()
    end)

    -- ===== Notifications =====
    local function notify(text)
        local n = Instance.new("TextLabel")
        n.Size = UDim2.new(0,220,0,42)
        ...
        TweenService:Create(n, TweenInfo.new(0.3), {
            Position = UDim2.new(1,-240,1,-110)
        }):Play()

        task.wait(2)

        TweenService:Create(n, TweenInfo.new(0.3), {
            Position = UDim2.new(1,-240,1,0)
        }):Play()
