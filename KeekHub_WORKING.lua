...
local function saveConfig()
        ...
        if writefile then pcall(function() writefile("KeekPC.json",HS:JSONEncode(cfg)) end) end
end
...
local function buildGui()
        ...
        local old = game:GetService("CoreGui"):FindFirstChild("KeekHub");if old then old:Destroy() end
        local pg = LP:FindFirstChild("PlayerGui");if pg then local o2=pg:FindFirstChild("KeekHub");if o2 then o2:Destroy() end end

        local gui = Instance.new("ScreenGui")
        gui.Name="KeekHub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
...
-- ===== PREMIUM OPEN/CLOSE SYSTEM =====
closeBtn.Size=UDim2.new(0,26,0,26)
closeBtn.Position=UDim2.new(1,-34,0.5,-13)
closeBtn.BackgroundColor3=Color3.fromRGB(120,20,35)
...
miniToggleBtn.Size = UDim2.new(0,50,0,50)
miniToggleBtn.Position = UDim2.new(0,20,0.5,0)
miniToggleBtn.BackgroundColor3 = Color3.fromRGB(180,20,40)
miniToggleBtn.Text = ""
miniToggleBtn.Visible = false
Instance.new("UICorner", miniToggleBtn).CornerRadius = UDim.new(1,0)

local openIcon = Instance.new("ImageLabel", miniToggleBtn)
openIcon.Size = UDim2.new(0.6,0,0.6,0)
openIcon.Position = UDim2.new(0.2,0,0.2,0)
openIcon.BackgroundTransparency = 1
openIcon.Image = "rbxassetid://6031091004"
...
local function rotateIcon(icon)
    icon.Rotation = 0
    TS:Create(icon, TweenInfo.new(0.25), {Rotation = 180}):Play()
end
...
local function rippleEffect(parent)
    ...
    TS:Create(ripple, TweenInfo.new(0.4), {
...
local function showGui()
    ...
    TS:Create(main,TweenInfo.new(0.25,Enum.EasingStyle.Back),{Size=UDim2.new(0,300,0,426)}):Play()
    miniToggleBtn.Visible=false
end
...
-- ===== END PREMIUM SYSTEM =====
        ...
        titleLbl.Text="KEEK";titleLbl.TextColor3=C_RED_ACC;titleLbl.Font=Enum.Font.GothamBlack;titleLbl.TextSize=15
        titleLbl.TextXAlignment=Enum.TextXAlignment.Left;titleLbl.ZIndex=6
        -- REPLACED WITH PREMIUM BUTTON
local closeBtn = Instance.new("TextButton",header)
        closeBtn.Size=UDim2.new(0,26,0,26);closeBtn.Position=UDim2.new(1,-34,0.5,-13);closeBtn.BackgroundColor3=Color3.fromRGB(40,10,15)
        closeBtn.BorderSizePixel=0;closeBtn.Text="X";closeBtn.TextColor3=C_RED_ACC
        closeBtn.Font=Enum.Font.GothamBlack;closeBtn.TextSize=13;closeBtn.ZIndex=50;closeBtn.AutoButtonColor=false
        Instance.new("UICorner",closeBtn).CornerRadius=UDim.new(0,6)
        local closeBtnStroke=Instance.new("UIStroke",closeBtn);closeBtnStroke.Color=C_RED_ACC;closeBtnStroke.Thickness=1.2
        ...
        closeBtn.MouseEnter:Connect(function() TS:Create(closeBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(80,15,25),TextColor3=C_WHITE}):Play() end)
        closeBtn.MouseLeave:Connect(function() TS:Create(closeBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(40,10,15),TextColor3=C_RED_ACC}):Play() end)
        ...
        local function mkToggle(pg,label,defKey,defOn,onToggle,onKeyChanged)
                ...
                if defKey then
                        ...
                        keyBtn.BackgroundColor3=Color3.fromRGB(22,8,12);keyBtn.BorderSizePixel=0;keyBtn.Text=defKeyName
                        ...
                        local kbStroke=Instance.new("UIStroke",keyBtn);kbStroke.Color=Color3.fromRGB(90,20,30);kbStroke.Thickness=1
                        keyBtn.MouseEnter:Connect(function() TS:Create(keyBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(50,15,22)}):Play() end)
                        keyBtn.MouseLeave:Connect(function() TS:Create(keyBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,8,12)}):Play() end)
        ...
        laggerBox=mkInput("Main","Lagger Speed",LS,function(v) if v>0 and v<=500 then LS=v end;saveConfig() end)
        do
                ...
                btn.BackgroundColor3=Color3.fromRGB(22,8,12);btn.BorderSizePixel=0;btn.TextColor3=C_WHITE
                btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.ZIndex=6
                Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
                local btnStroke=Instance.new("UIStroke",btn);btnStroke.Color=Color3.fromRGB(90,20,30);btnStroke.Thickness=1
                btn.MouseEnter:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(50,15,22)}):Play() end)
                btn.MouseLeave:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,8,12)}):Play() end)
        ...
        do
                ...
                local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,72,0,24);btn.Position=UDim2.new(1,-82,0.5,-12)
                btn.BackgroundColor3=Color3.fromRGB(22,8,12);btn.BorderSizePixel=0;btn.Text=(KB.DropBrainrot.kb or KB.DropBrainrot.gp or Enum.KeyCode.Unknown).Name;btn.TextColor3=C_WHITE;btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.ZIndex=6
                Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
                local dbStroke=Instance.new("UIStroke",btn);dbStroke.Color=Color3.fromRGB(90,20,30);dbStroke.Thickness=1
                btn.MouseEnter:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(50,15,22)}):Play() end)
                btn.MouseLeave:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,8,12)}):Play() end)
                ...
                local function stopListen(key)
                        ...
                        if key then btn.Text=key.Name;prevText=key.Name
                                if tostring(key):find("Button") then KB.DropBrainrot.gp=key;KB.DropBrainrot.kb=nil else KB.DropBrainrot.kb=key;KB.DropBrainrot.gp=nil end;saveConfig() end
                end
                btn.MouseButton1Click:Connect(function()
                        if listening then stopListen(nil);return end
                        prevText=btn.Text;listening=true;_anyKeyListening=true;btn.Text="..."
        ...
        do
                ...
                mkLbl(row,"TP Down")
                local btn=Instance.new("TextButton",row);btn.Size=UDim2.new(0,72,0,24);btn.Position=UDim2.new(1,-82,0.5,-12)
                btn.BackgroundColor3=Color3.fromRGB(22,8,12);btn.BorderSizePixel=0;btn.Text=(KB.TPFloor.kb or KB.TPFloor.gp or Enum.KeyCode.Unknown).Name;btn.TextColor3=C_WHITE;btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.ZIndex=6
                Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
                local tpStroke=Instance.new("UIStroke",btn);tpStroke.Color=Color3.fromRGB(90,20,30);tpStroke.Thickness=1
                btn.MouseEnter:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(50,15,22)}):Play() end)
                btn.MouseLeave:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,8,12)}):Play() end)
                local listening=false;local lconn;local prevText=(KB.TPFloor.kb or KB.TPFloor.gp or Enum.KeyCode.Unknown).Name
                local function stopListen(key)
                        listening=false;_anyKeyListening=false;if lconn then lconn:Disconnect();lconn=nil end;btn.Text=prevText
                        if key then btn.Text=key.Name;prevText=key.Name
        ...
        do
                ...
                guiKeyBtn.BackgroundColor3=Color3.fromRGB(22,8,12);guiKeyBtn.BorderSizePixel=0;guiKeyBtn.TextColor3=C_WHITE;guiKeyBtn.Font=Enum.Font.GothamBold;guiKeyBtn.TextSize=10;guiKeyBtn.ZIndex=6
                Instance.new("UICorner",guiKeyBtn).CornerRadius=UDim.new(0,5)
                local gkStroke=Instance.new("UIStroke",guiKeyBtn);gkStroke.Color=Color3.fromRGB(90,20,30);gkStroke.Thickness=1
                guiKeyBtn.MouseEnter:Connect(function() TS:Create(guiKeyBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(50,15,22)}):Play() end)
                guiKeyBtn.MouseLeave:Connect(function() TS:Create(guiKeyBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(22,8,12)}):Play() end)
        ...
        -- ===== RIGHT-SIDE QUICK ACCESS PANEL (ALL BLACK) =====
        local QB_SIZE = UDim2.new(0,80,0,44)
        local QB_PAD = 5
        local QB_COLS = 2
        local QB_BG = Color3.fromRGB(6,6,6)
        local QB_BTN_BG = Color3.fromRGB(14,5,8)
        local QB_BTN_HOV = Color3.fromRGB(30,10,16)
        local QB_BTN_ON = Color3.fromRGB(40,10,18)
        local QB_TEXT = Color3.fromRGB(255,255,255)
        local QB_STROKE = Color3.fromRGB(70,15,25)

        local qbDefs = {
                {label="DROP\nBR",      action=function() runDrop() end,                        toggle=false},
                {label="AUTO\nLEFT",    action=nil,                                              toggle=true,
                        getState=function() return autoLeftEnabled end,
                ...
                {label="AUTO\nBAT",     action=nil,                                              toggle=true,
                        getState=function() return autoBatEnabled end,
                        onToggle=function(on)
                                autoBatEnabled=on
                                if on then
                                        if autoLeftEnabled then autoLeftEnabled=false;if autoLeftSetVisual then autoLeftSetVisual(false) end;stopAutoLeft() end
                ...
                {label="AUTO\nRIGHT",   action=nil,                                              toggle=true,
                        getState=function() return autoRightEnabled end,
                        onToggle=function(on)
                                autoRightEnabled=on
                                if on then
                                        if autoLeftEnabled then autoLeftEnabled=false;if autoLeftSetVisual then autoLeftSetVisual(false) end;stopAutoLeft() end
                ...
                {label="TP\nDOWN",      action=function() runTPFloor() end,                      toggle=false},
                {label="CARRY\nSPEED",  action=nil,                                              toggle=true,
                        getState=function() return speedMode end,
                        onToggle=function(on)
                                speedMode=on;if modeValLbl then modeValLbl.Text=on and "Carry" or "Normal" end
                        end},
                {label="BAT\nCOUNTER",  action=nil,                                              toggle=true,
                        getState=function() return medusaCounterEnabled end,
                        onToggle=function(on)
                                medusaCounterEnabled=on;if on then setupMedusa(LP.Character) else for _,c in pairs(medusaConns) do pcall(function() c:Disconnect() end) end;medusaConns={} end
                                if setMedusaVisual then setMedusaVisual(on) end
                        end},
                {label="LAGGER\nMODE",  action=nil,                                              toggle=true,
                        getState=function() return stretchRezEnabled end,
                        onToggle=function(on)
                                stretchRezEnabled=on;if on then enableStretchRez() else disableStretchRez() end
                                if setStretchRezVisual then setStretchRezVisual(on) end
                        end},
        }

        local QB_ROWS = math.ceil(#qbDefs/QB_COLS)
        local QB_W = QB_COLS*(QB_SIZE.X.Offset)+((QB_COLS+1)*QB_PAD)
        local QB_H = QB_ROWS*(QB_SIZE.Y.Offset)+((QB_ROWS+1)*QB_PAD)

        local qbFrame = Instance.new("Frame",gui)
        qbFrame.Name="QuickAccessRight"
        qbFrame.Size=UDim2.new(0,QB_W,0,QB_H)
        qbFrame.Position=UDim2.new(1,-QB_W-8,0.5,-QB_H/2)
        qbFrame.BackgroundColor3=QB_BG
        qbFrame.BorderSizePixel=0
        qbFrame.Active=true
        Instance.new("UICorner",qbFrame).CornerRadius=UDim.new(0,9)
        local qbStroke=Instance.new("UIStroke",qbFrame)
        qbStroke.Color=QB_STROKE;qbStroke.Thickness=1
        makeDraggable(qbFrame)

        local qbBtnVisuals = {}

        for i,def in ipairs(qbDefs) do
                local col = ((i-1)%QB_COLS)
                local row = math.floor((i-1)/QB_COLS)
                local btn = Instance.new("TextButton",qbFrame)
                btn.Size=QB_SIZE
                btn.Position=UDim2.new(0,QB_PAD+col*(QB_SIZE.X.Offset+QB_PAD),0,QB_PAD+row*(QB_SIZE.Y.Offset+QB_PAD))
                btn.BackgroundColor3=QB_BTN_BG
                btn.BorderSizePixel=0
                btn.Text=def.label
                btn.TextColor3=QB_TEXT
                btn.Font=Enum.Font.GothamBlack
                btn.TextSize=10
                btn.AutoButtonColor=false
                Instance.new("UICorner",btn).CornerRadius=UDim.new(0,9)
                local bs=Instance.new("UIStroke",btn);bs.Color=QB_STROKE;bs.Thickness=1

                if def.toggle then
                        local function refreshQBBtn()
                                local on = def.getState and def.getState() or false
                                TS:Create(btn,TweenInfo.new(0.15),{BackgroundColor3=on and QB_BTN_ON or QB_BTN_BG,TextColor3=on and Color3.fromRGB(180,20,40) or QB_TEXT}):Play()
                                bs.Color = on and Color3.fromRGB(100,10,20) or QB_STROKE
                        end
                        qbBtnVisuals[i]=refreshQBBtn
                        btn.MouseButton1Click:Connect(function()
                                local newState = not (def.getState and def.getState() or false)
                                if def.onToggle then def.onToggle(newState) end
                                refreshQBBtn()
                        end)
                else
                        btn.MouseButton1Click:Connect(function()
                                TS:Create(btn,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(40,40,40)}):Play()
                                task.delay(0.16,function() TS:Create(btn,TweenInfo.new(0.12),{BackgroundColor3=QB_BTN_BG}):Play() end)
                                if def.action then def.action() end
                        end)
                end
                btn.MouseEnter:Connect(function() TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=QB_BTN_HOV}):Play() end)
                btn.MouseLeave:Connect(function()
                        local on = def.toggle and def.getState and def.getState() or false
                        TS:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=on and QB_BTN_ON or QB_BTN_BG}):Play()
                end)
        end

        -- Sync QB panel toggle button states on RunService
        RunService.Heartbeat:Connect(function()
                for i,fn in pairs(qbBtnVisuals) do fn() end
        end)
        -- ===== END RIGHT-SIDE QUICK ACCESS PANEL =====
...
local function loadConfig()
        if not(isfile and isfile("KeekPC.json")) then return end
        local ok,cfg=pcall(function() return HS:JSONDecode(readfile("KeekPC.json")) end)
...
local function preloadKeybinds()
        ...
        if cfg.laggerSpeed then LS=cfg.laggerSpeed end
        if cfg.tpMode and (cfg.tpMode=="Manuel" or cfg.tpMode=="Semi" or cfg.tpMode=="Medusa") then
                tpMode=cfg.tpMode;medusaMode=(tpMode=="Medusa")
        end
end
